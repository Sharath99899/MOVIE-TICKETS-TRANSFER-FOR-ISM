...
def BuyerAddCropsToCart(request):
    ...
    rate= price*75/100
    rate=float(rate)
    BuyerCropCartModels.objects.create(buyerusername=buyerUser,buyeruseremail=buyeremail,sellername=sellername,cropname=cropname, description=description, price=price, file=file,status=cartStatus, rate=rate)
...
def BuyerTotalAmountCheckOut(request):
    ...
    data1 = BuyerCropCartModels.objects.values()
    print(type(data1))
